...
while(theta <= pi/2)    %calculating the beam solid angle
    F = sin(theta)*(abs(0.5*(cross(cross(l_e_h, s_i_h), s_i_h)+cross(l_h_h, s_i_h))).^2)*d_phi_f/10; %normalized pattern function of E field
...
solid_angle = 2*pi*norm(integrand); %sr, beam solid angle
E_field_max = 1; %V/m, max of the feed E field
...
ylabel('Co-polairzed Pattern [dBi]');title('Figure 3');grid on;